...
### Start up #################################
	...
	mkdir -p /usr/local/src/magento/tmp
...
### Installer: Sample-Data ########################	   
    ...
    then
		...
		if [ -f ${versionspath}/magento-sample-data-1.2.0.tar.gz ];
		then
			echo [-] Sample data archieve already downloaded
		else
	                echo [-] loading Sample-Data from Magento servers
			wget http://www.magentocommerce.com/downloads/assets/1.2.0/magento-sample-data-1.2.0.tar.gz -o ${versionspath}/magento-sample-data-1.2.0.tar.gz	
		fi
